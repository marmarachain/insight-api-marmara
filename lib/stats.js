...
// constructor
const Amountstats = function(amountstats) {
  this.Height = amountstats.Height;
  this.CalculatedTotalNormals = amountstats.CalculatedTotalNormals;
  this.CalculatedTotalActivated = amountstats.CalculatedTotalActivated;
  this.CalculatedTotalLockedInLoops = amountstats.CalculatedTotalLockedInLoops;
  this.BlockTime = amountstats.BlockTime;
};

Amountstats.getAll = result => {
  sql.query("SELECT Height, CalculatedTotalNormals, CalculatedTotalActivated, CalculatedTotalLockedInLoops, BlockTime FROM marmarastat", (err, res) => {
    ...
    console.log("marmaraamountstatsall: ", res);
    result(null, res);
  });
};

Amountstats.getLast = result => {
  sql.query("SELECT Height, CalculatedTotalNormals, CalculatedTotalActivated, CalculatedTotalLockedInLoops, BlockTime FROM marmarastat ORDER BY Height DESC LIMIT 1", (err, res) => {
    ...
    console.log("marmaraamountstats: ", res);
    result(null, res);
  });
};

module.exports = Amountstats;
...
StatsController.prototype.generateStatsTotals = function() {
  ...
  var blockDate = new Date(new Date(this.cache.computed.marmaraAmountStatByBlocksDiff[this.cache.computed.marmaraAmountStatByBlocksDiff.length - 1].time * 1000).getFullYear() + '-' + (new Date(this.cache.computed.marmaraAmountStatByBlocksDiff[this.cache.computed.marmaraAmountStatByBlocksDiff.length - 1].time * 1000).getMonth() + 1 < 10 ? ( '0' + (new Date(this.cache.computed.marmaraAmountStatByBlocksDiff[this.cache.computed.marmaraAmountStatByBlocksDiff.length - 1].time * 1000).getMonth() + 1)) : new Date(this.cache.computed.marmaraAmountStatByBlocksDiff[this.cache.computed.marmaraAmountStatByBlocksDiff.length - 1].time * 1000).getMonth() + 1) + '-' + (new Date(this.cache.computed.marmaraAmountStatByBlocksDiff[this.cache.computed.marmaraAmountStatByBlocksDiff.length - 1].time * 1000).getDate() < 10 ? '0' + new Date(this.cache.computed.marmaraAmountStatByBlocksDiff[this.cache.computed.marmaraAmountStatByBlocksDiff.length - 1].time * 1000).getDate() : new Date(this.cache.computed.marmaraAmountStatByBlocksDiff[this.cache.computed.marmaraAmountStatByBlocksDiff.length - 1].time * 1000).getDate())).toISOString().substr(0, 10);
